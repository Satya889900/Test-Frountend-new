        ...
        <div style={{ padding: "16px 14px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
            ...
            {showPhoto && photo ? (
              ...
              <div style={{ width: 58, height: 58, borderRadius: "50%", background: "#9ca3af" }} />
            )}
          </div>
        ...
        <div style={{ background: "#0b5568", color: "#e2e8f0", padding: "18px 14px" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: "10px" }}>
            {showPhoto && photo ? (
              ...
              <div style={{ width: 86, height: 86, borderRadius: "50%", background: "rgba(255,255,255,0.25)" }} />
            )}
          </div>
        ...
        <div style={{ marginLeft: 26 }}>
          <div style={{ display: "grid", gridTemplateColumns: "106px 1fr", gap: 12, marginBottom: 10 }}>
            <div style={{ marginTop: 12, width: 58, height: 58, border: `2px solid ${accent}`, background: "#fff", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {showPhoto && photo ? <img src={photo} alt="Profile" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: 8, color: "#6b7280" }}>PHOTO</span>}
            </div>
    ...
    return (
      <div style={{ fontFamily: font, minHeight: 700, background: "#fff", padding: "20px" }}>
        <div style={{ borderBottom: `3px solid ${accent}`, marginBottom: 12, paddingBottom: 10 }}>
          ...
          <div style={{ fontSize: 9, color: "#64748b" }}>{email} {phone ? `| ${phone}` : ""} {linkedin ? `| ${linkedin}` : ""}</div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div>{visibleExperience.length > 0 && <Section title="Experience">{visibleExperience.map((e, i) => <div key={i} style={{ fontSize: 10, marginBottom: 6 }}>{e.jobtitle} - {e.company}</div>)}</Section>}</div>
          <div>{visibleEducation.length > 0 && <Section title="Education">{visibleEducation.map((e, i) => <div key={i} style={{ fontSize: 10, marginBottom: 6 }}>{e.degree}</div>)}</Section>}</div>
        </div>
      </div>
    );
  }
